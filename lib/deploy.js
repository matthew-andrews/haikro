	...
	return readFile(project+"/Procfile", { encoding: 'UTF-8' })
		...
		.then(function(script) {
			return fetch("https://api.heroku.com/apps/"+app+"/slugs", {
					...
					body: JSON.stringify({ process_types: { web: 'export PATH="node_modules/.bin:node-linux-x64/bin:$PATH"; ' + script }, commit: commit })
				}).then(function(response) {
					return response.json();
				}).then(function(slug) {
					if (slug.id === 'forbidden' || slug.id === 'unauthorized' || slug.id === 'not_found') {
