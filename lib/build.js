...
function minimumNpmVersion() {
	return exec('npm --version')
		.then(function(version) {
			...
			logger.debug('checking NPM version ' + version + ' matches ^2.7.0');
...
function ensureNodeDownloaded(opts, version) {
	logger.debug('matching node version against: "'+version+'"');
	return fetch("https://semver.io/node/resolve?range="+version)
		.then(function(response) { return response.text(); })
		.then(function(version) {
			var node = 'node-v'+version+'-linux-x64';
			logger.debug('checking if node '+version+' already downloaded');
...
function ensureIojsDownloaded(opts, version) {
	logger.debug('matching iojs version against: "'+version+'"');
	return fetch("https://semver.herokuapp.com/iojs/resolve?range="+version)
		.then(function(response) { return response.text(); })
		.then(function(version) {
			var iojs = 'iojs-v'+version+'-linux-x64';
			logger.debug('checking if iojs '+version+' already downloaded');
			return exists(opts.cwd+'/'+iojs)
				.then(function(exists) {
					if (exists) {
						...
						return exec('curl https://iojs.org/dist/v'+version+'/iojs-v'+version+'-linux-x64.tar.gz | tar xz iojs-v'+version+'-linux-x64/bin/iojs iojs-v'+version+'-linux-x64/bin/node', opts);
					}
				})
				.then(function() {
...
function getDependencies(opts) {
	return Promise.all([
			...
			exec('npm ls --production --parseable', opts),
...
module.exports = function(options) {
	...
	return Promise.all([
		...
		.then(function(results) {
			logger.info('making tarball', '.haikro-cache/slug.tgz');
			...
			var command = "tar "+tarOptions+" -f .haikro-cache/slug.tgz " + excludeList.join(' ') + ' ' + includeList.join(' ');
