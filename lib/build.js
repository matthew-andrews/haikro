...
function ensureNodeDownloaded(opts) {
	return exec("node -e \"var pkg; try { pkg=require('./package.json'); } catch(err) {} process.stdout.write(pkg && pkg.engines && pkg.engines.node ? pkg.engines.node : '');\"", opts)
...
module.exports = function(project) {
	...
	return Promise.all([
		...
		.then(function(results) {
			...
			var hasSlugIgnore = results[1];
			return exec("tar "+tarOptions+" -f tmp/slug.tgz" + (hasSlugIgnore ? " --exclude-from=.slugignore" : "" ) + " --exclude './tmp' .", opts);
		})
		.then(function() {
