...
module.exports = function(opts) {
	...
	return exists(opts.cwd+'/.haikro-cache/slug.tgz')
		...
		.then(function(releases) {
			if (releases.message === 'Bad credentials' || releases.message === 'Not Found') {
				throw new Error('cannot access github with provided token, are your github access token and repository correct?');
			}
			logger.verbose("checking if release already exists in array: " + JSON.stringify(releases));
		...
		.then(function(release) {
			logger.verbose("uploading slug", "uploads.github.com/repos/"+repository+"/releases/"+release+"/assets");
