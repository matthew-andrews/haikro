...
module.exports = function(opts) {
	...
	fetch("https://api.github.com/repos/"+repository+"/releases", { headers: authorizedHeaders })
		...
		.then(function(releases) {
			...
			if (release[0]) return release[0].id;
		})
		.then(function(release) {
			if (!release) throw new Error('release not found'); // todo try to make a release from git tag
			return release;
		})
		.then(function(release) {
